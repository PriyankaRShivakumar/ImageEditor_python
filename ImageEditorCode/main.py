...
class FrontEnd:
    ...
    def __init__(self, master):
        self.master = master #this will contain the root that was passed. 
        #self.master is visible in the entire class unlike master so we set master to self.master
...
#"""**********************************Header********************************************"""
        ...
        self.logo = PhotoImage(file='python_logo.gif').subsample(3, 3)
         #instead of pack, we can use grid to pass the location. another method is place that require exact coordinates like x,y. These are geometry managers
        ttk.Label(self.frame_header, image=self.logo).grid(row=0, column=0, rowspan=2)
        ttk.Label(self.frame_header, text='Welcome to the Image Editor App!').grid(row=0, column=1, columnspan=1)
        ttk.Label(self.frame_header, text='Upload, edit and save your images Easily!').grid(row=1, column=1, columnspan=1)
#"""**********************************End of Header********************************************"""
#"""**********************************Main Menu********************************************"""
        self.frame_menu = ttk.Frame(self.master)
        self.frame_menu.pack()
        #the below command adds border to the frame. RIDGE is a type of border. Padding adds the spacing between the frames
        self.frame_menu.config(relief=RIDGE, padding=(50, 15))
        #With sticky,the widget is centered in its cell in the provided direction. SW is southwest.
...
#"""**********************************End of Main Menu********************************************"""
#"""**************************************Canvas********************************************"""
        self.canvas = Canvas(self.frame_menu, bg="gray", width=300, height=400) #providing width and height will fix the size
        self.canvas.grid(row=0, column=1, rowspan=10)
#"""**********************************End of Canvas********************************************"""
#"""**********************************Footer Menu********************************************"""
        self.apply_and_cancel = ttk.Frame(self.master)
        self.apply_and_cancel.pack()
        #grid can also be applied as below
...
#"""**********************************Triggers for Adjust Menu Options********************************************"""
    def brightness_action(self, value):
        self.filtered_image = cv2.convertScaleAbs(self.filtered_image, alpha=self.brightness_slider.get())
        self.display_image(self.filtered_image)
...
root = Tk()
# Here instantiated root window is passed to FrontEnd class and it is stored in master. So root is called master in the FrontEnd class
app = FrontEnd(root)
root.mainloop()